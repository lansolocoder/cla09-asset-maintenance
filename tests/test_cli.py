...
class PlanCommandTests(unittest.TestCase):
    ...
    def add_asset(self, asset_id: str = "A001") -> None:
        result = self.invoke(
            "asset", "add", "--id", asset_id, "--name", "投影仪",
            "--cost", "3200", "--purchased-at", "2026-09-01",
        )
    ...
    def add_plan(
        ...
        plan_id: str = "P001",
        asset_id: str = "A001",
        interval_days: str = "30",
        first_due: str | None = None,
    ) -> subprocess.CompletedProcess[str]:
        if first_due is None:
            first_due = self.today.isoformat()
        return self.invoke(
            "plan", "add", "--id", plan_id, "--asset", asset_id,
            "--interval-days", interval_days, "--first-due", first_due,
        )
    ...
    def test_due_rolls_forward_by_interval(self) -> None:
        self.add_asset()
        first_due = self.today - timedelta(days=10)
        self.assertEqual(self.add_plan(interval_days="30", first_due=first_due.isoformat()).returncode, 0)
        next_due = first_due + timedelta(days=30)  # 20 days in the future
        self.assertEqual(self.due(self.today.isoformat())["due"], [])
        payload = self.due(next_due.isoformat())
        self.assertEqual(len(payload["due"]), 1)
        self.assertEqual(payload["due"][0]["due_date"], next_due.isoformat())

    def test_due_sorting_and_status(self) -> None:
        self.add_asset("A001")
        self.add_asset("A002")
        earlier = self.today.isoformat()
        self.assertEqual(self.add_plan("P002", "A001", "30", earlier).returncode, 0)
        self.assertEqual(self.add_plan("P001", "A002", "30", earlier).returncode, 0)
        self.assertEqual(
            self.invoke("asset", "status", "--id", "A001", "--to", "repairing").returncode,
            0,
        )
        payload = self.due(self.today.isoformat())
        self.assertEqual([entry["plan_id"] for entry in payload["due"]], ["P001", "P002"])
        self.assertEqual(payload["due"][1]["asset_status"], "repairing")

    def test_retired_asset_plans_never_listed(self) -> None:
        self.add_asset()
    ...
    def test_add_rejections_leave_no_trace(self) -> None:
        self.add_asset()
        self.assertEqual(self.add_plan().returncode, 0)
        future = (self.today + timedelta(days=1)).isoformat()
        rejections = [
            ("--id", "P002", "--asset", "NOPE", "--interval-days", "30", "--first-due", "2026-01-01"),
            ("--id", "P001", "--asset", "A001", "--interval-days", "30", "--first-due", "2026-01-01"),
            ("--id", "P002", "--asset", "A001", "--interval-days", "0", "--first-due", "2026-01-01"),
            ("--id", "P002", "--asset", "A001", "--interval-days", "-5", "--first-due", "2026-01-01"),
            ("--id", "P002", "--asset", "A001", "--interval-days", "abc", "--first-due", "2026-01-01"),
            ("--id", "P002", "--asset", "A001", "--interval-days", "1.5", "--first-due", "2026-01-01"),
            ("--id", "P002", "--asset", "A001", "--interval-days", "30", "--first-due", "2026-13-01"),
            ("--id", "P002", "--asset", "A001", "--interval-days", "30", "--first-due", "not-a-date"),
            ("--id", "P002", "--asset", "A001", "--interval-days", "30", "--first-due", future),
        ]
        for extra in rejections:
            with self.subTest(extra=extra):
        ...
        payload = self.due(self.today.isoformat())
        self.assertEqual([entry["plan_id"] for entry in payload["due"]], ["P001"])

    def test_due_rejects_invalid_as_of(self) -> None:
        for bad in ["not-a-date", "2026-13-01", "2026-1-1"]:
            with self.subTest(bad=bad):
                result = self.invoke("plan", "due", "--as-of", bad)
                self.assertEqual(result.returncode, 1)
                self.assertTrue(result.stderr.startswith("error: "), result.stderr)
                self.assertEqual(result.stdout, "")

    def test_missing_arguments_are_an_error(self) -> None:
        self.add_asset()
        result = self.invoke("plan", "add", "--id", "P001")
        self.assertNotEqual(result.returncode, 0)
        self.assertEqual(result.stdout, "")
        result = self.invoke("plan", "due")
        self.assertNotEqual(result.returncode, 0)
        self.assertEqual(result.stdout, "")
