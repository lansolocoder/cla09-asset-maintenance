...
def _parse_interval_days(text: str) -> int:
    try:
        interval = int(text)
    except ValueError:
        raise LedgerError(f"invalid interval days: {text!r} (expected a positive integer)") from None
    if interval <= 0:
        raise LedgerError(f"invalid interval days: {text!r} (expected a positive integer)")
    return interval
...
def _parse_first_due(text: str) -> date:
...
def _next_due_date(first_due: date, interval_days: int, today: date) -> date:
    """Roll ``first_due`` forward by whole periods to the first date not before ``today``."""
    if first_due >= today:
        return first_due
    elapsed = (today - first_due).days
    periods = -(-elapsed // interval_days)  # ceil division
    return first_due + timedelta(days=periods * interval_days)
...
def _cmd_plan_add(args: argparse.Namespace) -> int:
    interval_days = _parse_interval_days(args.interval_days)
    first_due = _parse_first_due(args.first_due)
    conn = _connect()
    try:
        with conn:  # rolls back if any check raises
            if conn.execute(
                "SELECT 1 FROM assets WHERE id = ?", (args.asset,)
            ).fetchone() is None:
                raise LedgerError(f"asset {args.asset} not found")
            if conn.execute(
            ...
            try:
                conn.execute(
                    "INSERT INTO plans (id, asset_id, interval_days, first_due)"
                    " VALUES (?, ?, ?, ?)",
                    (args.id, args.asset, interval_days, first_due.isoformat()),
                )
            except sqlite3.IntegrityError:
...
def _cmd_plan_due(args: argparse.Namespace) -> int:
    ...
    try:
        plans = conn.execute(
            "SELECT id, asset_id, interval_days, first_due FROM plans"
        ).fetchall()
        due = []
        today = date.today()
        for plan_id, asset_id, interval_days, first_due in plans:
            status = _current_status(conn, asset_id)
            if status is None or status == "retired":
                continue
            next_due = _next_due_date(date.fromisoformat(first_due), interval_days, today)
            if next_due > as_of:
                continue
            due.append(
                {
...
def build_parser() -> argparse.ArgumentParser:
    ...
    plan = subparsers.add_parser("plan", help="manage maintenance plans")
    ...
    plan_add.add_argument("--asset", required=True, help="asset identifier")
    plan_add.add_argument(
        "--interval-days", required=True, help="maintenance interval in days (positive integer)"
    )
    plan_add.add_argument(
        "--first-due",
        required=True,
    ...
    plan_due = plan_subparsers.add_parser(
        "due", help="list plans due on or before a cutoff date as JSON"
    )
    plan_due.add_argument("--as-of", required=True, help="cutoff date, YYYY-MM-DD")
    plan_due.set_defaults(handler=_cmd_plan_due)
