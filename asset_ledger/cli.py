...
def build_parser() -> argparse.ArgumentParser:
    ...
    plan_add.add_argument(
        "--next-due", default=None, help="下次到期日期（YYYY-MM-DD）"
    )
    plan_add.add_argument(
        "--cycle-days",
        dest="cycle_days",
        default=None,
        help="维保周期天数（正整数），需与 --start-date 一起使用",
    )
    plan_add.add_argument(
        "--start-date",
        dest="start_date",
        default=None,
        help="周期起算日期（YYYY-MM-DD），到期日期为起算日期后第 N 天",
    )
    plan_add.add_argument(
        "--request-id",
        dest="request_id",
    ...
    plan_list = subparsers.add_parser(
        "plan-list", parents=[sub_common], help="列出指定资产的全部维保计划"
    )
    _add_tag_arg(plan_list)
    plan_list.set_defaults(func=cmd_plan_list)

    plan_due = subparsers.add_parser(
        "plan-due", parents=[sub_common], help="按基准日查询到期或即将到期的维保计划"
    )
    plan_due.add_argument(
        "--as-of",
        dest="as_of",
        required=True,
        help="基准日（YYYY-MM-DD），当天算到期",
    )
...
def cmd_plan_add(args: argparse.Namespace, out, err) -> int:
    ...
    try:
        try:
            plan_id, tag, name, next_due = db.add_plan(
                conn,
                tag=args.tag,
                name=args.name,
            ...
            # A failed first plan must not leave an empty file behind.
            if not existed:
                conn.close()
                closed = True
                db_path.unlink(missing_ok=True)
    ...
    print(f"资产标签: {tag}", file=out)
    print(f"下次到期日期: {next_due}", file=out)
    return 0
