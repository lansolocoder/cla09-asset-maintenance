...
为已登记资产建立维保计划，到期日期二选一：`--next-due` 直接给出下次到期日期，
或用 `--cycle-days`（正整数周期天数）加 `--start-date`（起算日期），到期日期为
起算日期后第 N 天并保存该周期；两者同时提供或都缺失、周期非正整数均为参数错误：

```bash
python3 -m asset_ledger plan-add --tag NB-001 --name "除尘保养" --next-due 2026-10-01
python3 -m asset_ledger plan-add --tag NB-001 --name "电池检测" \
  --cycle-days 90 --start-date 2026-09-01
```

- 成功（退出码 0）按行输出计划编号、计划名称、资产标签和下次到期日期。
- 计划名在同一资产下唯一、精确匹配判重，不同资产可同名。
- 可携带 `--request-id` 安全重试：同标识且资产标签、计划名称与到期输入完全一致时，
  重复提交返回首次的相同输出和同一计划编号；同标识不同内容拒绝并说明原因；
  失败的尝试不占用该标识。
- 资产不存在、必填缺失、日期不合法、计划名重复均非零退出，错误写入 stderr，
  不改动资产、既有计划和变更历史；失败的登记不会创建此前不存在的数据文件。

按资产查看全部计划（按计划名升序，无计划时输出“尚无维保计划”）：

```bash
python3 -m asset_ledger plan-list --tag NB-001
```

按基准日查询全部资产的到期情况：到期日期不晚于基准日（当天算到期）记“已到期”，
晚于基准日不超过 7 天记“即将到期”，其余不列出；每行含资产标签、计划名称、
到期日期、状态和剩余天数（到期日期−基准日，当天 0，逾期为负），按剩余天数升序、
再按台账编号升序、再按计划名升序排列；无匹配时输出“无到期或即将到期的计划”：

```bash
python3 -m asset_ledger plan-due --as-of 2026-09-24
```
